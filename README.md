...
4. **Stable Audio Open Small** — licensed under the [Stability AI Community License](https://stability.ai/community-license-agreement). Gated on HuggingFace (no ungated mirror exists), so install is a one-time manual step: download `model.safetensors` and `model_config.json`, then T5ynth picks them up via *Auto-Scan* or *Browse...* in Settings.
