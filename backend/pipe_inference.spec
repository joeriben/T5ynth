...
# The oscillator's library. lco_write.py reads both of these with a plain open()
# relative to its own __file__, which inside a frozen bundle is _MEIPASS — so
# without them here the first LRO prompt raises FileNotFoundError and the
# oscillator is dead in every installer, while working perfectly from source.
# SPECPATH, not a bare relative path: the build runs from backend/ locally and
# from the workflow's working-directory in CI, and this must not depend on cwd.
for _lco_data in ('lco_library.json', 'dco_lexicon.json'):
    _lco_path = Path(SPECPATH) / _lco_data
    if not _lco_path.exists():
        raise SystemExit(
            f"{_lco_data} is missing from {SPECPATH}. It is generated by "
            "tools/lco_build_library.py; a bundle without it cannot run the "
            "language-resonant oscillator at all."
        )
    datas += [(str(_lco_path), '.')]
...
# t5-base is no longer bundled — it is acquired by the user via T5ynth's
# Settings page (Apache-2.0, ungated, ~430 MB installer savings).
