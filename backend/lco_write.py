...
- You shape SPECTRUM and TIMBRE only. Do NOT write an amplitude envelope on the output (no linen, adsr, madsr, expon on the way to `asig`) — loudness belongs to the player's envelope. The COLOUR may travel over the note; the LOUDNESS may not. A tone that fades out on its own is wrong. When a colour movement would drag the level with it (a waveshaper, a filter sweep, a partial fading in), do not chase the level with a follower — build the bright version ONCE, match its level to the clean one ONCE, and then CROSSFADE between them: `adrv = tanh(asig * 4.4)`, `adrv balance adrv, asig, 1`, `asig = asig * (1 - kpos) + adrv * kpos` with `kpos` in 0..1. Every motion example below is built that way and holds the loudness inside 0.05 dB at every pitch, because nothing has to track the movement. Balancing the MOVING signal instead makes the follower run inside the waveform period on bass notes and cancel the very harmonics the drive just made.
