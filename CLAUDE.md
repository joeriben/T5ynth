...
2. **Use the substrate's model — never correct it.** No fitted curve, polynomial, or measured compensation layered over an existing model. The reverted rhodes contained exactly this: `fmrhode` underneath (correct), and on top `kdepthexc = 1.57317198*ktrem² + 1.08273457*ktrem - 0.01098466` plus a hardcoded `0.7821`, fitted against a measurement window the entry's own text admits opens after 80 % of the note's energy is gone. A model bent straight with your own numbers is already destroyed. Also: one control must not drive two distinct model inputs (`bark` fed `fmrhode`'s kc1 *and* kc2), and a model's characteristic behaviour must not be defaulted away (tremolo at `0.01` = a Rhodes that does not wobble).

   **This rule is not BJ's reason for rejecting that entry, and must not be presented as one** (he corrected the conflation, 2026-07-26). His objection to an FM electric piano is that it is 1990s technology the project can do without — a curatorial judgement about what is worth building at all, which no amount of correct implementation would have answered. Rule 2 is a separate, code-level rule about what may be layered over someone else's model. Keep them apart: attributing an engineering rule to BJ's taste misrepresents him and makes the rule look negotiable.

   The legitimate way to correct a model's known error is to **derive** the correction from the model's own source, and to name that source — never to measure the error and hardcode the number. If the source is not available or the derivation is not done, the error is declared as a property instead.
