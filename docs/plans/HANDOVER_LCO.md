...
8. **CLOSED 2026-07-25 — all twelve axes on those three entries are settable and every one moves the sound.** Re-measured at 220 Hz over each axis's own anchors, power-weighted centroid: `fm_ep` ting 551→336 Hz, ring 384→1413, hollowness 339→412, strike 292→353; `drum_head` pitched 433→322, strikepos 418→545, tension 297→505, damping 390→267; `analog_osc` wave 228→525, drive 474→378, fat 474→529, age 497→450 — worst max-sample difference 0.199 (`analog_osc.drive`), the rest 0.5 to 1.33. `lco_axis_probe.AXIS` matches all twelve; the finding below was written before those three entries were rebuilt, and the variable names are simply not the axis names (`kpitch` for `pitched`, `khollow` for `hollowness`), which is what an earlier grep read as "no axis line". `fm_ep`'s `ring` has a mechanism and a 1029 Hz centroid travel. **The rest of this item is the historical record, not the current state.**

   ~~Nine declared axes on three entries have never been measured, because they have no axis line to set.~~ 39 of the 42 parametrised entries declare each axis as one line the probe can substitute (`kmus = 0.0 ; musette [0..1]: …`). The three oldest bake theirs into expressions instead, so `lco_axis_probe`/`lco_param_audit` cannot touch them: `drum_head`'s four (`pitched`, `strikepos`, `tension`, `damping`), `analog_osc`'s four (`wave`, `drive`, `fat`, `age`) and `fm_ep`'s `ring`. Nobody has ever checked that any of the nine is a colour control rather than a volume fader — the check that has already caught a shipped defect once. Worse than unmeasured: **`fm_ep`'s `ring` has no mechanism at all**, not in the body and not in its eleven `anchor_code` variants; the author is told "ring [0..1] short/medium/long" about a body with no decay to lengthen. `drum_head`'s three `pitched` variants are also stale — they predate the `balance` its `code` now ends on, so they hand the author back the 9 dB register tilt the entry itself fixed.
...
9. **CLOSED 2026-07-25 — author-facing text that does not match its own code.** All four claims were checked; three were real and are fixed, one was a misreading.

   - `organ`'s `principal` is three ranks, not one (`054beafe`). The gloss was corrected rather than the code, because the code **cannot** simply zero the upper ranks: the one quadratic through (0,0), (0.5,1), (1,1.7) keeps the default and the full registration bit-identical and holds loudness to 0.026–0.112 dB, and it makes `principal` a single rank whose only motion is its own 0.061 Hz wander — **moves=False at coherence 0.00**, against True at 1.00 today. What moves this entry is three incommensurate wanders (0.061 / 0.083 / 0.107 Hz) beating against each other. A truthful `principal` and movement-by-default cannot both hold in this body as written; **that trade is BJ's**.
   - `jaw_harp`'s `pluck` did almost nothing — 0.0245 of whole-note difference and 122 Hz of colour across its entire range, under a top anchor promising "the frame buzzing with it". Fixed (`d70b8693`) by making the mix quadratic ABOVE the shipped default and leaving everything at and below it bit-identical: colour across the four anchors 1159 / 1200 / 1459 / 1772 Hz against 1159 / 1191 / 1243 / 1281, comb contrast 69.6 → 50.4 dB against 69.6 → 63.6.
   - The FM trio's `detune = 0` (`f4fc2013`). At detune=0 the colour still travels 0.72 / 1.56 / 1.33 octaves on `fm` / `fm_bell` / `metallic_fm` and reads moves=True at coherence 1.00. What the axis removes is the amplitude BEAT — level swing 0.7 / 0.0 / 0.0 dB against 6.7 / 5.6 / 3.0 at `gentle`. Stillness is not on offer on this platform, so the gloss promised what the author cannot have.
   - **The "1.30 dB from `free_reed`" was a loudness reading, not a timbral one, and it is not a redundancy finding.** Two entries at the same loudness is the platform working. Spectrally they are 18.10 / 21.11 / 24.75 / 23.93 dB rms apart at 110 / 220 / 440 / 880 Hz — different instruments by a wide margin. Worth noting that the `pluck` fix brings them closer at that axis's top (7.2–7.8 dB apart at pluck = 1.0), which is what a hard-flicked jaw harp should do.
...
1. **CI ships no Csound — one decision away.** Locally this is done: a macOS build with Csound present now copies `CsoundLib64`, the 24 dependency-free plugin opcode modules and the whole dependency tree into `Contents/libs` of the .app, .vst3 and .component (`tools/bundle_csound_macos.sh`, wired as a POST_BUILD step in `CMakeLists.txt`, `T5YNTH_BUNDLE_CSOUND=ON`), rewrites every load command to `@loader_path/../libs`, ships the LGPL text and re-signs. A build WITHOUT Csound is unchanged, as `CMakeLists.txt:65` requires. Proven, not assumed: `tools/verify_csound_bundle.py` (static sweep, then a sandbox with both Homebrew prefixes denied, then compile+perform a real library idiom) and `tools/verify_lro_in_standalone.py` (an authored orchestra out of a .t5p, played in the BUILT Standalone with Homebrew Csound cut off — 227 Hz, peak 0.55; `--prove-it-can-fail` shows the app cannot even start without the bundle).

   What is missing is only the CI's own Csound. The four options were measured and put to BJ on 2026-07-26; he has not decided yet, so nothing was wired into `.github/workflows/build.yml`:
   `brew install csound dylibbundler` in the macOS job (one line, Homebrew's build, 76 runtime deps of install time, and the version table in `NOTICE.txt` would have to follow Homebrew rather than be maintained by hand) · vendoring the ~10 MB tree in git (deterministic, exact versions, binaries in the repo) · a pinned upstream `Csound-MacOS-universal-6.18.1.dmg` (45.6 MB, upstream provenance, self-containment unverified) · leaving it, which is this blocker.

   Also undecided: whether the hardened runtime should carry `com.apple.security.cs.disable-library-validation`. Without it, LGPL relinking freedom is theoretical for a signed release — a user's own build of `CsoundLib64` will not load into the notarized app.
