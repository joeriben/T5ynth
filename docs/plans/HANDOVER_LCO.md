...
1. **„Crystal singing bowls in a cathedral" comes out extremely dissonant** (BJ, 2026-07-29): „Wäre gut da eine eigene Parametrisierung zu machen (oder Instrument)". What the ground under it actually is — measured, because the first reading of it was wrong:

   - **`crystal` is not one of the 51 sound words** and appears in no surface form.
   - The dedicated `singing_bowl` entry was one of the 40 deleted on 2026-07-26. **But the prompt is not unanchored**: `metallic_fm` carries `singing bowl` and `klangschale` as surface forms *and in its `why`* — „gong / tam-tam / anvil / singing bowl / metallic percussion: true inharmonic metal partials (h 1.73/3.19/4.51) written as 2-operator FM at a 2.41 ratio" — and `named_entries("crystal singing bowls in a cathedral")` returns `['metallic_fm']`. So the author is pointed at a **deliberately inharmonic FM gong**, which is a plausible source of "extrem dissonant" and the first thing to check.
   - That also sets the comparison target: `nearest_existing.wav` is `metallic_fm`, not `struck_bar`. A bowl is a modal body; the honest route is `mode` or `barmodel` with published bowl ratios, per `CLAUDE.md` → Instrument Authoring — method and source in writing first, the A/B on disk before the new orchestra, BJ's ear deciding. Whether the answer is a new entry or an axis on `metallic_fm` is open.
