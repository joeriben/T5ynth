...
- **4 × `M7` register tilt** — `drum_head` **9.0 dB across 110–880 Hz (−2.99 dB/octave)**, `driven_metal` 8.2 / −2.72, `cymbal` 6.6 / −2.21, `struck_bar` 6.0 / −2.00, all against 0.0 dB of render-to-render scatter. Loudness following the keyboard is a defect on any of them and `driven_metal` is the one built since the revert.
...
- **`kpw` must FOLD at 0 and 1, not wrap.** Same spectrum either way, so the sound travels the same path — but a wrap makes `vco2` jump across its own table discontinuity and emit a lone one-sample transient (−0.74 in a stream sitting at +0.60, in the raw opcode output before any DC handling). At 0.5 Hz that is one isolated transient per second, which is what knocks; the narrow pulse itself repeats 220 times a second and is a buzz. **Read the number with its caveat:** "15 wrapping, 0 folding over 8 s at full depth" was measured for `610d3286`, on the body that still had the hardwired swing sweeping the whole 0..1 range; the detector was never committed, and an independent reconstruction on a hand-driven sweep got 10 / 26, disagreeing in direction. Re-measure with a committed detector before relying on the count. **The mechanism is not in doubt and the fold is not dead code**, but in today's body the shipped settings never reach it — `kwidth + kjit` stays inside [0.008, 0.992] at `age` = 1 — so it guards a movement source the AUTHOR writes past the ends, which is exactly what a prompt asking for 0 %…100 % produces.
...
0. **The sound words move onto the synth's filter, and the synth's filter grows what they need.** BJ, 2026-07-31, after the measurement in item 2 below: *„Klangwörter ist das Thema nun"*, and the architectural ruling that decides the shape of it — **„Filter gehören dem Synth. Wenn wir also neue weitere Filtermodelle brauchen, oder Parametrisierungen von Filtern, dann gehören die in den Synth-Code, nicht in die Bib, mit Verweis auf die Bib."** So a sound word stops carrying `tone`/`atone` lines in its Csound and instead SETS the synth's filter. Four things follow, in order of what they cost:

   - **Step one costs no new DSP.** `filterType`, `filterCutoff`, `filterResonance`, `filterAlgorithm`, `filterWarpStyle`, `filterSlope` and `filterDrive` are already on the author shelf (`authorParamShelf()`, `src/PluginProcessor.cpp`). A word expressed as a filter SETTING makes two words a conflict the author resolves; two words appending two filters stack silently, which is exactly the defect in item 2.
   - **It is one filter, not a bank.** `nasal` wants a resonance near 1.5 kHz with the rest left alone; today only a global tilt exists. The published models for brightness and sharpness are computed *per critical band* (`docs/LCO_TIMBRE_SEMANTICS.md` §1.3), so a bank is what the models already assume.
   - **The roughness window has no modulator at all — and a modulator is NOT to be built for it.** Roughness is envelope fluctuation at **15–300 Hz** (Daniel & Weber 1997). Drift runs 1/128–2 Hz, LFO1 0.01–30 Hz (`src/PluginProcessor.cpp:1548`, `:1469`); both offer S&H as a wave, and both stop far below the band. BJ raised a construction for the low-frequency case — *„mehrere fix konfigurierte S&H-LFO übereinander, darunter mindestens einen kontinuierlichen … Denk mal LKW, Erdbeben → überlagernde tieffrequente Wellen. Nicht einmal dasselbe wie eine S&H"* — and then withdrew it the same hour: **„lohnt ggf. den Aufwand nicht. wir haben t5osc für so was."** So a superimposed low-frequency field is not a build order; the T5 oscillator covers that class of sound. Whatever carries texture for the sound words has to come from somewhere other than a new synth modulator.
   - **Topology and age are TWO axes, and conflating them was wrong.** BJ, 2026-07-31: *„es sind technisch unterschiedliche Filtertopologien. Das hat erst mal nichts mit analog/digital zu tun."* SVF / Ladder / Warp are structures. **`old` is a second axis made entirely of ERRORS** — his own list: *„Fluktuationen von Pitch und Peak, Vorverstärker, Eigenrauschen und Verstimmung der Tuning-Potis"*. He rates it *„ja, gute Idee, ist aber noch nicht vorgesehen"*, and notes *„unser ladder ist perfekt gestimmt - nice"*: on an analog ladder the resonance peak's level and its self-oscillation pitch move with the cutoff, which is what had to be trimmed to keep the peak playable across a few octaves; a zero-delay-feedback structure is exact there. So the error would be deliberately reintroduced — which under Instrument Authoring rule 2 must be **derived from a documented model source** or **declared as a property with a range where it holds**, never a measured curve laid over the correct filter.

   Not started. The `FilterWarpStyle` key that carried a product name was renamed to `algebraic` on 2026-07-31, with a compatibility reader (`filterWarpStyleFromString`, `src/PluginProcessor.cpp`) so stored presets keep resolving; forward compatibility is the open half — a preset written today reads as `Tanh` on every build already in users' hands.

   **SURVEYED 2026-07-31, and the survey moved the order of work.** Two inventories, both against the tree as it stands:

   **(a) The 51 sound words.** 24 are pure filter (`tone`/`atone`/`reson` plus a mix — `dark` is the single line `asig tone asig, 1200`), 15 are filter plus something else, 12 carry no filter at all (ring modulation, waveshaping). **None of the 51 `why` fields names a source or a measurement**; every one is a declared convention, and thirteen of them are defined only relative to another entry („softer than mellow", „the inverse of sparse"). So the move onto the synth's filter has a clean first tranche of 24 and no measured ground under any of them — `docs/LCO_TIMBRE_SEMANTICS.md` is where that ground would have to come from.

   **(b) Nine of the 51 are not modifiers at all — they are oscillators, and they overwrite the instrument.** `warm`, `hollow`, `nasal`, `fat`, `thin`, `thick`, `reedy`, `distorted`, `old`. Their code does not take `asig` as an input; it builds a new one from `vco2` and ends `asig = adrv0 * (1 + kagw0)`. Whatever stood in `asig` — the instrument, any filter word applied before — is gone. All nine use the same variable names (`adrv0`, `kagw0`, `kpw0s0`, …), so two of them also destroy each other. `old`'s own `why` promises „purely spectral"; its code is a `vco2` with a tanh drive. Item 2 below establishes that the author pastes these library blocks verbatim, so this is live, not hypothetical. **This is a defect and it is independent of the filter move** — it has to be decided and repaired first, because moving the 24 filter words is pointless while nine other words can erase the result.

   **(c) The gate decides what „a word sets the filter" can even mean.** The author's parameter shelf is 152 ids and includes all eleven filter parameters (`authorParamShelf()`, `src/PluginProcessor.cpp:6818`); the wire format is a `SET: <id> = <value>` line inside the fence after `READING`, parsed by `read_settings` (`backend/lco_write.py:626`) and re-validated in `applyAuthorSettings` (`src/PluginProcessor.cpp:6977`). But the shelf is behind the **KNOBS** switch, `lco_sets_params`, **created default OFF** (`src/PluginProcessor.cpp:1437`, comment: „the knobs are the player's until the player says otherwise"), and when it is off the author is not even shown that the controls exist — `render_params` returns `""`. **So a sound word expressed as a synth-filter setting does nothing at all in the default patch.** Today `dark` at least darkens, because its filter travels inside the orchestra. Two readings of BJ's ruling follow from this and they are different builds: the words drive the player's existing filter (free, but silent unless the player turns KNOBS on), or the synth grows an author-owned timbre filter stage that the library only points at (new DSP, works by default, leaves the player's filter to the player). The second is the literal reading of *„neue Filtermodelle … gehören in den Synth-Code, mit Verweis auf die Bib"* and is what is recommended; it is BJ's call and is not started.

1. **BUILT AND APPROVED 2026-07-31 — „Crystal singing bowls in a cathedral" comes out extremely dissonant** (BJ, 2026-07-29): „Wäre gut da eine eigene Parametrisierung zu machen (oder Instrument)".

   The cause was the routing: `named_entries("crystal singing bowls in a cathedral")` returned `['metallic_fm']` and nothing else, because that entry carried `singing bowl` and `klangschale` as surface forms. `metallic_fm` is a Chladni free circular plate — a gong, and a good one — whose declared exponent range [1.4, 2.4] cannot produce a bowl's mode ratios (2.83 and 5.42 would need p = 2.57 and p = 2.44, two different exponents, both outside it), and at its defaults its loudest low partial sits at 62 Hz under a played 220. So it was a new entry, not an axis: **`singing_bowl`**, a `mode` bank on the (j,0) shell modes with every mode a measured pair, `bowl` / `warble` / `ring` / `sung`. Method, both sources and the acceptance gate: `docs/plans/PLAN_lco_singing_bowl.md`, written before the first orchestra line. `singing bowl` and `klangschale` moved off `metallic_fm`, which keeps gong / tam-tam / anvil / bell metal.

   BJ, 2026-07-31, on the parameter page: **„ja, dsie bowls sind sehr gut! so lassen. verfiziertes instrument"**.

   **Two level findings stay open, and they are not what he heard** — the audition page put one gain over the whole set, so neither could be audible there. The body is louder low by 12.61 dB from 110 to 880 Hz (−4.20 dB/octave), which is `mode`'s own Q/f gain law and is in the library's other mode banks too (`driven_metal` −2.72, `drum_head` −2.99). At 220 Hz the post-onset peak is 2.049, under the transparent ceiling; at 110 Hz it is 3.994, over the absolute 2.75, so a bass note is clipped away. Both are declared in the entry's `why`. A correction is one scale factor against `kc` and would leave the spectrum at every pitch untouched — but it changes what the body does away from 220 Hz, which is BJ's call and is why it was not applied.
...
8. **`rate` left the entry without a separate word from BJ.** The plan listed removing it as needing his say-so; what he gave was approval of the *sound* after the A/B. It had nothing left to set once the hardwired swing was gone, and the movement rate is now something the author writes. The exemplar it copies is in the `width=moving with wave=1` anchor block: `klfo oscili 0.5, 0.5` — 0.5 cps, **two seconds a round trip**. That is the only rate an author is shown *as a movement*; the entry's own bodies also carry three drift oscillators at 0.043, 0.057 and 0.7 Hz, which are `age` and belong to item 5.
...
21. **Disk under `tools/`, re-measured 2026-07-29** (the old figures were wrong in both directions): the render directories `tools/*_out/` hold **578 MB** and every byte of that is *gitignored*, so none of it shows in `git status`; everything ignored under `tools/` is **856 MB**, and the whole `tools/` tree on disk is **854 MB** — the "over 1 GB" never existed. What actually shows as untracked-and-not-ignored under `tools/` is **18.6 MB**, mostly one-off scripts. Deleting anything under `tools/` is BJ's call. Note before deciding: `backend/dco_frames.py` is untracked but **is imported by the tracked test** `tools/test_dco_author.py`.
