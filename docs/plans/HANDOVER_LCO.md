...
- **A third thing `balance` costs: the note's start depends on how long the plugin has been open.** Its averager has no note-on reset, so the gain it happens to hold at the edge is a function of uptime. `struck_bar`, `glass`, `cymbal`, `tanpura`, `string` and the FM family all read it; 29 shipped bodies carry a `balance`, and eight of them are ones only this rule made visible (`fm`, `fm_bell`, `fm_ep`, `drum_head`, `metallic_fm`, `struck_bar`, `cymbal`, `glass`). It is REPORTED, not gated: §4 has no rule about it, and the only cure is to key what must happen after the attack to `knote`, which the host does reset.
  ...
  ⚠ **The per-body Hz figures that stood here are withdrawn, not corrected.** They were carried over from a verification agent's run rather than measured on the shipped tool, and an independent pass reproduced none of them (it read `glass` 340 where this said 303 and `string` 498 where this said 163 — which reverses the conclusion the sentence drew). The grid they were taken on has since been replaced as well: it spanned 2.7 s, less than one period of the five slowest generators in the library, and aliased on `hurdy_gurdy`'s 1.45 Hz wheel. `lco_axis_probe` now spans the slowest shipped period. **Re-measure before quoting any number for this class** — `.venv/bin/python tools/lco_axis_probe.py --body <b> --all --gate`.
