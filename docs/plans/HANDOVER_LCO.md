...
- **A third thing `balance` costs: the note's start depends on how long the plugin has been open.** Its averager has no note-on reset, so the gain it happens to hold at the edge is a function of uptime. Measured across four settled instance ages against a zero noise floor: `struck_bar` 450 Hz of first-window colour, `glass` 303, `tanpura` 186 — more than the free-running k-rate LFO the detector was written for (`string`, 163). Eight shipped bodies are in this position. It is REPORTED, not gated: §4 has no rule about it, and the only cure is to key what must happen after the attack to `knote`, which the host does reset.
...
9. **Movement fails at named anchors away from 220 Hz in 7 of the 13 new entries** — `ocarina` at `breath=soft` + `chiff=clean` in 4 of 6 registers, `hurdy_gurdy` at its own defaults at 55 Hz. The gate only ever asked at 220.
