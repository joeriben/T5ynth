...
### The bowl was struck once when the orchestra was built, never by a note

Found by the adversarial pass over the repair above, and it is the more serious of the two. `mpulse kamp, 0` emits **one** impulse at the instance's first sample, and the host scores a single always-on instance (`i 1 0 360000`). The first draft got away with it because `exp(-knote / 0.0018)` was a real per-note strike sitting next to it; moving the weight to a-rate removed that and left `mpulse` as the entire exciter — so the shell was struck at compile time and every note after it only un-gated the decaying residue.

Post-onset peak at 220 Hz with the defaults, at the plugin's rate, by how long the instance had been alive when the note began:

| instance age | 0 s | 0.5 s | 2 s | 8 s | 30 s |
|---|---|---|---|---|---|
| before | 1.6132 | 1.3694 | 0.8837 | 0.3634 | **0.0193** |
| after | 1.6132 | 1.1588 | 1.0958 | 1.4330 | **1.6059** |

38 dB decided by nothing but how long the plugin had been open, and at `ring` 0.0 the third note was digital silence. The repair is `plucked_wire`'s construction, mirrored rather than invented — that entry carries the same `reinit` and a comment naming the same trap: `if changed2(ktrig) == 1 then reinit BOWLSTRIKE`. The residual ±1.7 dB in the second row is the mode pair's beat phase, not a decay.

Three more, from the same pass:

- **`sqrt` of a negative NaN'd the whole voice** at `sung` outside 0…1. `perform_check` judges Csound's exit code, which was 0, so it would have shipped and reached the DAW buffer. `limit kbld, 0, 1`.
- **The lock-in layer had no Nyquist gate** while the shell bank had one, so at 1× oversampling its 3rd and 4th harmonics folded (at kfreq 11000: a 100 Hz tone under an 11 kHz note). Same `kg`-style gate as the shell modes.
- **`metallic_fm`'s `why` still advertised „singing bowl"** — the surface forms moved, the sentence the author actually routes on did not, and the index printed the claim directly above the entry that contradicts it.

### What the repairs did to the measurements

| | approved draft | now |
|---|---|---|
| gate failures | 6 | **2** |
| register tilt | 12.61 dB, −4.20 dB/oct | **4.34 dB, −0.08 dB/oct** |
| headroom over the ceiling | 61 of 328 corners | **none** |
| `moves` failures | 3 | **none** |
| rate factor 44100 → 176400 | 6.09× | **~1.0×** |
| level travel: `bowl` / `warble` / `ring` / `sung` | — | 0.30 / 3.56 / 1.16 / 6.7 dB |

`ring` moving 1.16 dB is what the entry claims — the strike really is the same at every Q — and it only became true once the strike was per-note. The 6.7 dB on `sung` is one strike against no strike; the two layers are now levelled through the note instead (−15.10 against −15.77 dB rms over four seconds).

### Still open

- **The corner space is 8.01 dB wide at 220 Hz and 12.67 dB across corners and registers together**, over the one-loudness bound of 3.00 dB. Most of it is `sung`. Declared, not flattened.
- **The re-hearing, and it is not a formality.** The approval of 2026-07-31 is recorded in the entry as `OFFEN`, because it was given on a page rendered at 44100 for a body that has since been repaired three times. `tools/lco_listening/singing_bowl_params/index.html` — generated by `tools/lco_param_page.py`, all four axes at their anchors, 108 combinations, 12 s, one gain for the set, **rendered at 4× the way the plugin does** — is the page the entry now has to survive.
