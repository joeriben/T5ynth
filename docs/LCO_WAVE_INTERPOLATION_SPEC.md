...
**NACHTRAG (BJ-Ohr-Verdikt, 2026-07-14, nach Slice 2a):** Der multiplikative Spektral-Tilt (Stärke 0.3) als Gegenteil-Endpunkt ist ohrgeprüft **zu subtil** („ausgesprochen subtil, über MacBook-Speaker kaum unterscheidbar" — nur Säge→Glocke, der Fall mit echter spektraler Distanz, bestand). Ersetzt durch `_opposite_endpoint`: **absolute Amplituden-Rampe** (Floor 0.25 → 1.0 über die h-Ränge, hell mit 0.8-Spitzenabschwächung) **plus Serien-Erweiterung** für dünne Sätze (< 16 Partials): bis 4 Fortsetzungs-Partials oberhalb des obersten h (Lücken-Muster extrapoliert, Wachstum [1.0, 1.6], Reichweite ≤ 2.8 × oberstes h), im dunklen Zustand per Union stumm (a=0) — Energie **wandert** in Partials, die vorher nicht klangen, exakt der Mechanismus des ohr-bestandenen handgebauten Aufblühens (772→2901 Hz). Harmonische Quellen bleiben harmonisch (Erweiterung rastet aufs Ganzzahl-Raster — keine Umleitung des Bake-Pfads). Sinus-Atmen vertieft: 0.72 → **0.35** (−2.9 dB war unhörbar, −9.1 dB ist eine Geste). Messwirkung Glocke „opening up": Centroid-Hub 633→675 Hz (Tilt) ⇒ 710→1636 Hz (Rampe+Erweiterung).
