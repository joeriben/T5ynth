...
**Folgerung.** Der Homöostat funktioniert im *deterministischen* Regime sauber (sigma = monotoner Aktuator). Das *stochastische* Regime — das die Auto-Variation des Plugins trifft — lockert den Attraktor **nicht**, sondern erschwert die Regelung (Rauschen schluckt das Stellsignal). Konkrete Design-Konsequenz für die spätere C++-Verdrahtung: PI statt P + Messfilterung; das Band ist eng und die Strecke langsam. Quantifiziert nebenbei die **Loop-Attraktor-Stärke** des i2i-Loops, gegen die der Homöostat das Gegengift ist.
