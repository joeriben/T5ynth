...
>
> **Gebaut, 2026-07-29** (`PromptPanel::triggerDcoReprompt`). Ein STEP: Probe-Render der zuletzt authored Orchestra über `CsoundEngine::renderBareOscillator` — nackt, ohne ADSR und Filter, 220 Hz / 3 s / Gate zu bei 2,5 s — dann `analyze(probe, …, topk 5)`, dann derselbe Stance-Aufruf wie bisher. Drei Dinge, die dabei nicht selbstverständlich waren: (1) **Zwei Raten, und das ist der Punkt.** Gerendert wird bei `hostRate × LRO-Oversampling` (Default 4×) — die Engine kompiliert denselben Text bei dieser Rate, und ein Body darf seine Partialzahl oder seinen FM-Index aus `sr` ableiten, ein 1×-Probe wäre also nicht nur dreckiger, sondern kann ein **anderer Klang** sein. *Gehört* wird bei der Hostrate: `renderBareOscillator` dezimiert vor der Rückgabe durch dieselben Halfband-Stufen wie `renderUpTo` im Livepfad (`decimateBy`). Ohne das beschriebe das Ohr ein Band, das niemand hört — die Spektralwörter rechnet das Backend auf dem nativen Signal gegen absolute Hz-Schwellen (Centroid 500/1500/3500), und gemessen auf diesem Build wandert der Centroid eines alias-reichen Saw-Stacks von 6766 Hz (dezimiert) auf 28384 Hz (192 kHz roh); der alte 1×-Probe lag bei 8474 Hz, die Differenz zu 6766 ist genau das heruntergefaltete Aliasing. (2) Tags und Spektralwörter bleiben **getrennt und getrennt beschriftet** („Neural ear:" / „Spectral:"), wie beim neuralen Zwilling — links eine Assoziation aus festem Vokabular, rechts eine Rechnung; zusammengeworfen wiegt das Modell beides gleich. Die alten Labels des LCO-Turns („Machine reading:", „The machine read it as:") sind weg, sie behaupteten eine Lektüre des Codes. (3) **Kein Rückfall auf die READING-Zeile.** Wer nicht hören kann, tritt nicht: sonst stünde die Selbstauskunft des Autors unter einem Label, das Hören behauptet. Gegattert wird auf die Orchestra (nicht auf die READING-Zeile, die bei SNAP-Recall leer sein darf) und gegen ein laufendes Csound-Compile-Fenster.
>
> **Nachtrag 2026-07-29 — der Auto-Loop ist gebaut, und die Stopp-Ästhetik ist keine neue.** Die Übersetzungstabelle unten führt „Loop-Trigger: Drift-Regen-Zyklus" als *optional*, die offenen Punkte fragen „Auto-Loop ja/nein" und verlangen dafür eine Stopp-Ästhetik („transcribe terminiert von selbst; abduction nie"). Gebaut als `PromptPanel::pollLcoRepromptCadence` auf demselben 10-Hz-Timer wie der neurale Zwilling. Entscheidend: **eine Taktquelle für beide Paradigmen** — dieselbe REGENERATE-Schaltbox (`drift_regen`, Manual / a.s.a.p. / 1–16 Takte, mit `driftRegenBpm`), dieselbe Takt-Arithmetik. Getrennt bleibt, was die Paradigmengrenze trennt: die HALTUNG (`dco_reprompt_stance` vs. `reprompt_stance`), nicht die Uhr; ein zweiter Taktregler wäre ein UI-Element, das niemand verlangt hat, und zwei Uhren, die auseinanderlaufen können.
>
> Getaktet wird `triggerDcoReprompt`, nie `triggerDcoBake`: ein Takt ist immer „hören → umschreiben → backen". Ohne Haltung gibt es nichts zu treten, und ein Durchfallen aufs Backen hieße, dieselbe Eingabe im Takt neu zu würfeln und dabei ein funktionierendes Instrument zu ersetzen.
>
> **Die Stopp-Ästhetik ist die des neuralen Loops, nicht eine eigene** — sie war schon da, die Frage von 2026-07-10 hat sie nur nicht als Antwort erkannt: (1) Manual ist autoritativ, der Loop tritt dann pro GENERATE-Druck; (2) jeder manuelle GENERATE — Knopf, XL CC 37 *und* die Return-Taste im Prompt-Feld — setzt `drift_regen` zurück auf Manual, hält also an, und zwar auch dann, wenn gerade ein Schritt läuft: sonst wäre eine laufende Kadenz nur noch an der Schaltbox zu stoppen, nie an dem Knopf, der sie gestartet hat; (3) eine passive Zustandswiederherstellung (DAW-Projekt öffnen, `_buffer.t5p` beim Start) zwingt **beide** Haltungen auf Off, dieselbe Anti-Überraschungsregel wie bei `seqRunning` — eine gespeicherte Haltung plus nicht-Manual wäre sonst ein selbstlaufender Generator, der die Orchestra überschreibt, mit der das Projekt gespeichert wurde, bevor der Nutzer irgendetwas angefasst hat. Ein deliberates Preset-LOAD stellt die Haltung weiterhin her (`importJsonPreset`). Fehlschläge (Ohr nicht erreichbar, Umschreibung leer) drosseln nur die Kadenz, 2 s, wie `lastRegenFailureMs_` — sonst würde a.s.a.p. bei totem Backend im Timertakt gegen den prozessweiten Csound-Lifecycle-Lock des Probe-Renders laufen.
>
...
| Loop-Trigger: Drift-Regen-Zyklus | **Gebaut 2026-07-29 als genau das: dieselbe REGENERATE-Schaltbox taktet beide Paradigmen** (s. Nachtrag). Daneben weiter der Schritt pro GENERATE-Druck, in Manual der einzige. **⚠ offen: ~1–2 s war der 1.5B; auf dem 12B ist der Stance-Schritt ungemessen, s. Nachtrag** |
...
- ~~**Auto-Loop ja/nein:** Der Einzelschritt ist der Kern. Ein Auto-Loop (Intervall- oder pro-Bake-Takt) ist trivial möglich, braucht aber eine Stopp-Ästhetik (transcribe terminiert von selbst; abduction nie).~~ **Erledigt 2026-07-29, s. Nachtrag:** gebaut auf der REGENERATE-Schaltbox; die verlangte Stopp-Ästhetik war die des neuralen Loops (Manual, GENERATE-setzt-zurück, Anti-Überraschung bei passiver Wiederherstellung).
