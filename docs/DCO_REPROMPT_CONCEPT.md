...
| `cleanPrompt` (Label-Echos, Längenkappung) | identisch übernehmbar (gleiche 1.5B-Schwächen) |
...
| Loop-Trigger: Drift-Regen-Zyklus | Schritt-Trigger neben BAKE; Auto-Loop optional (Bake ist ~ms, Qwen ~1–2 s — der Loop wäre *schneller* als der neurale) |
...
Die geshippten Stances sind Text→Text-Transformationen; sie laufen **unverändert** auf DCO-Prompts, weil jeder Qwen-Output durch denselben Lexikon-Trichter muss (Guardrail-Invariante: der Router bleibt „router not author", keine Zahl entsteht außerhalb des Lexikons — der Loop kann die Validierung nicht umgehen, egal was Qwen schreibt). Aber ihre *Wirkung* verschiebt sich charakteristisch:
...
**Denkbarer siebter, DCO-nativer Stance — „Grenzgänger" (Arbeitstitel):** Nimm die Flags als Material — „mache das Nichtverstandene zum Zentrum des nächsten Prompts". Das ist die DCO-Fassung von Modus 3 (Kritik) aus `SEMANTIC_LOOP_AESTHETICS.md`, mit umgekehrter Stoßrichtung: Kritisiert wird nicht AudioSets Taxonomie, sondern die eigene. Der Loop pendelt dann an der Vokabulargrenze entlang, statt zu konvergieren. (Nur Konzept; ob ein 1.5B-Modell diese Haltung hält, wäre wie beim entkitscher-Reframe empirisch zu prüfen.)
...
Die Plattform-Invariante (gehaltene Note folgt der aktuellen Table per Regen-XFade-Crossfade; seit dem Motion-Transport-Fix auch über DCO↔neural-Flips klickfrei) macht den DCO-Loop **performativ**: Ein gehaltener Akkord wandert hörbar durch die Interpretationskette — jede Iteration ein Crossfade in die nächste Deutung, ohne Neuanschlag. Der neurale Loop hat das prinzipiell auch, aber mit Sekunden Latenz pro Glied; der DCO-Loop taktet in Qwen-Geschwindigkeit (~1–2 s) und wird damit zur *spielbaren* Geste statt zur Hintergrund-Evolution.
...
Der DCO-Router *benutzt Qwen bereits* (S2-Residue-Routing im selben Backend-Prozess). Es gibt kein neues Modell, kein neues Gating (`setQwenAvailable` deckt beide Fälle), keinen neuen IPC-Frame-Typ — der Stance-Schritt ist ein `interpret`-Aufruf (bestehende Op) gefolgt von einem `dco`-Aufruf (bestehende Op). Die StanceBar-Komponente existiert und ist APVTS-gebunden. Konzeptionell ist der DCO-Re-Prompt damit fast reine *Verdrahtung* plus die Kurationsentscheidungen oben — das Teuerste ist das Nachdenken, und das ist dieses Dokument.
