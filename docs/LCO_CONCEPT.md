...
- **Movement by default.** Every sound moves; only an explicit "static" request is delegated to the standing-tone escape hatch. There is a **second exemption, and only one**, authorized by BJ on 2026-07-25: the **event-texture class**, a body that declares `; MOVEMENT: TEXTURE` in its own code. The gate then reports its movement reading instead of failing on it, and everything else it checks still applies. His reasoning: a findable RATE in the colour track *„is the right definition for a sustained tonal body and may be the wrong one for a stochastic texture, where being alive means precisely that there is no rate to find"* — *„genau. Da haben wir nicht an Natursounds gedacht. Hiermit freigeschaltet. Ich muss ja ohnehin alle neuen instrumente reviewen."*
