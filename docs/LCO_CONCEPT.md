...
Concretely, today: the prompt goes to the author model (gemma-4-12B QAT 4-bit GGUF via llama.cpp) together with the library **index** rendered from `backend/lco_library.json` — every entry, what it is, and what its parameters do to the sound, with no Csound in it. The model answers by thinking about what the sound IS and naming the entries it wants opened; Python fetches exactly those and hands over their real Csound, and the model then writes the orchestra body itself: real Csound lines, not a set of keys for Python to assemble.

**The consultation belongs to the model, not to Python** (BJ, 2026-07-24): „Es geht eine Liste mit Instrumenten und sonischen Beschreibungen der Parameter in den Prompt. Thinking wird dann nicht-deterministisch entscheiden was im nächsten Zug dem LLM aus der Bibliothek zur Verfügung gestellt wird." The two turns are one authoring of one sound — the standing rule behind them is that it is strictly forbidden to constrain the LLM deterministically, in any form. Python recognises the names the model used; it never decides what the model may see. A reply that names nothing opens the whole library rather than a default set. `backend/lco_library.json` is assembled from `backend/dco_lexicon.json` by `tools/lco_build_library.py`. Its one contract: the body writes its output into `asig`. `lco_write.wrap()` puts that body inside a fixed host scaffold — `ksmps=64`, `nchnls=16`, one numeric `instr 1`, `ivoice = p4`, 16 always-on voice instances, per-voice channels (`gate/freq/vel/pres/timb/trig`) — and the compiler judges the result; a failure goes back to the model with Csound's own error. The plugin runs the compiled orchestra live against `CsoundLib64.framework`.
...
4. **Surface forms** — the words that name it. Validation canon; the model never sees them. Since the consultation moved to the model (§2) they are no longer a routing layer over the user's prompt: they are how Python recognises an entry the *model* asked for, in whatever words it used.
...
- **~~Colour may travel over the note; loudness may not.~~ — REPHRASED 2026-07-27 on BJ's ruling: the fault is a REDUNDANT envelope, not amplitude behaviour as such, and it is not applied as a threshold.** *„die wenden wir nicht zu starr an. Mein Punkt war eigentlich dass UNNÖTIGE Hüllkurven den Synth degradieren, weil dort welche sind. aber ‚Pluck' ist ja gerade eine Transiente in sich, also qua benennung. zB ‚Piano' ist keine Benennung eines spezifischen Hüllkurvenverhaltens. Die tiefe A-Seite eines 10-Meter-Flügels schwingt Minuten."* Three things follow, and the third retires a clause:
  - **The test is necessity.** The synth already has envelopes. What degrades it is a body that brings a SECOND one doing the same job — an amplitude shape laid over the sound that the player's envelope would have made anyway, and which now cannot be turned off. That redundancy is the fault. A body whose amplitude behaviour IS the sound is not redundant and never was: *„Pluck" ist qua Benennung eine Transiente in sich* — the transient is the thing being named, not a shape applied to it.
  - **A name never prescribes an envelope.** "Piano" is not a statement about decay; the low A of a ten-metre grand rings for minutes. So no entry owes a canonical amplitude behaviour to what it is called, and none is forbidden one for the same reason. This **withdraws** the clause "an instrument key may not fake a decay it does not physically have" from the bullet below: there is no single physical decay behind a name for an entry to be faithful or unfaithful to.
  - **The dB figure is a reading, not a verdict.** `lco_axis_probe` still measures the spread and still prints it, and it still FAILS a body outright where the level is the only thing an axis does — instrument 3 shipped with two colour controls that were volume controls (§5.8, −4 dB and 3–5 dB) and that must stay catchable. But where an axis moves the sound and moves the level with it, the number goes in front of BJ's ear instead of into a threshold. The mechanism is a declaration in the body, `; LOUDNESS: <axis>`, alongside `; MOVEMENT: TEXTURE` and `; DECAY: SELF` — for the same reason as those two: it cannot be granted by accident, it names which axis it covers, and it leaves the measurement visible instead of hiding it under a larger bound.
    - **What the declaration reaches, and on what evidence — both narrowed by adversarial review, and both are the load-bearing part.** It clears the corner rule (1.00 dB, read at the gate frequency) and NOT the one-loudness-at-every-register rule (3.00 dB), which stays unexempted as the backstop: granting it there let 36 dB of pure gain across a two-axis cube pass both rules on a trace of movement. And the evidence that an axis "does something else" must be SPECTRAL — the two centroid meters — not `lco_measure`'s beat meters or `comb_db`, all three of which amplitude alone can satisfy: an axis that was 2.4 dB of gain plus a 2 % tremolo cleared the guard while every harmonic stayed put to a tenth of a dB. A second amplitude envelope is the thing this section forbids, so amplitude may never be the evidence that excuses one.
  - **~~loudness may not travel within a STANDING tone~~ — WITHDRAWN 2026-07-28 on BJ's ruling, and it was the clause that did the damage.** *„die bezieht sich darauf dass die HÜLLKURVE nicht dem Synth weggenommen werden soll, SELBSTVERSTÄNDLICH NICHT auf natürlich-physikalische Bedingungen.“* The rule was always aimed at a body that INVENTS a second envelope and so takes the player's own away. It says nothing about level that follows from the physics and may never be applied to it: a pulse gets quieter the narrower it gets — that IS pulse width modulation — and six detuned saws add incoherently where one does not. The same holds for filter resonance, register, pick position, mode distribution, duty cycle. Flattening physics removes the thing that makes the sound what it is. **The occasion:** `analog_osc` shipped with the pulse's rms divided back out (`0.57735 / (2*sqrt(d(1-d)))`), justified in as many words as *„weil im Lexikon alle Klangkörper eine Lautstärke haben sollen“* — exactly the misreading. What remains forbidden is the INVENTED envelope, never its consequence. A body that fades is a declared class rather than a licence; colour travelling over the note was never in question.
- **~~A self-decay is acceptable ONLY where there is no other way~~ — SUPERSEDED 2026-07-20 by BJ's explicit decision. A self-decay is now a CHOICE THE PROMPT MAKES.** The old rule (BJ, 2026-07-19: „Hüllkurven bei Naturinstrumenten können ja ok sein, aber NUR dort wo es nicht anders geht") is replaced, not bent, and the replacement is the `wave` convention below. BJ authorised it in as many words — *„dann kannst Du die wieder freigegebenen Instrumente mit den envs implementieren"* — and confirmed the consequence for names when asked directly. **This is recorded as his decision because narrowing a platform invariant is never a side effect of implementing something else.** What survives unchanged: the synth still owns the envelope for every `wave` key, which is still most of the library; ~~an instrument key may not fake a decay it does not physically have~~ (**withdrawn 2026-07-27**, see the bullet above — a name does not prescribe an envelope, so there is no physical decay to be unfaithful to); and ~~loudness still may not travel *within* a standing tone~~ (**withdrawn 2026-07-28**, see above).
  ...
  - **RETIRED 2026-07-26, by the architecture and not by a change of mind — BJ's decision, asked and given.** The convention needed a routing layer that maps the user's words onto a catalogue key, because a modifier word only means something to something that reads the user's words. That layer is no longer in the LRO's path. `pipe_inference` mode `csound` calls `lco_write.build_csound_response`: ONE inference in which the author reads the prompt itself, names the entries it wants opened, and writes the orchestra. `surface_forms` are matched against **the author's own reply** and never against the prompt (`lco_write._lookup`: *"It is now run on the AUTHOR's OWN reply … Understanding the prompt is the model's job"*). There is nothing left for `wave` to be understood by, and the choice it was invented to offer — does this sound stand or does it die — is one the author already makes, from the prompt's own words.
    - **What replaces it is entry text, not a mechanism.** The two entries that sit on either side of the choice name each other: `plucked_wire` (struck once, rings down) says that a prompt wanting a held plucked tone belongs in `string`, and `string` says the opposite. The author picks. This is one layer above where `wave` would have sat and needs no grammar.
    - **What does NOT come back.** Everything §4 still says about the standing case stands: the synth owns the envelope wherever the body can stand (the clause on loudness within a standing tone that stood here was withdrawn 2026-07-28). (The third clause that stood here — a key may not fake a decay it does not physically have — was withdrawn on 2026-07-27 with the rephrasing above, and is not part of what the retirement preserves.) A body that fades is a declared class, not a licence — see `; DECAY: SELF` in `tools/lco_axis_probe.py` (`declares_decay`), which is checked at the attack and still fails a body that SWELLS.
    - **The per-key word trade in the bullet above is retired with it.** No key has to give up a word to a `wave` counterpart, because there are no two readings to disambiguate.
- **Pitch belongs to the synth.** An opcode that invents its own register is unusable however good it sounds. `kfreq` is k-rate and glides.
  - **Corrected 2026-07-30, on BJ's reading:** *„das ist ja auch eine zu starre formulierung. Resonatoren haben feste freqs, insofern. der geist der regel, ihre Intention, entspricht nicht ihrer formuleirung."* „Everything must track `kfreq`" forbade things this library already does correctly, so the one rule is three, and only the first is about pitch. (1) **Pitch** is `kfreq` times a ratio, never a number. (2) **A resonance may be fixed in Hz** — a formant, a body resonance, a noise band, an LFO rate belongs to the *body*, not to the note, and is marked where it is written, as `brass` (`; bell formant, FIXED in Hz`, 1200) and `strings` (`; body resonance, FIXED`, 460) already do. A body whose resonances are fixed has a range where it works, which is a property to declare (`fmvoice`, below) and not a scale error to fix. (3) **A landmark** in the spectrum — „above the 6th harmonic", „the top of the spectrum" — is a **ratio** to `kfreq`: 2200 Hz is the 40th harmonic at 55 Hz and the 2nd at 880 Hz, so as a fixed number it names a different thing at every pitch. **How far the rule and the library actually agree, counted rather than asserted:** all 28 instruments and all 17 motions are clean under it — every literal frequency in them is an annotated resonance or an LFO rate — and `shimmer`'s highpass corner and the six words whose added fundament sat at a fixed 220 Hz were changed to match. The **adjectives are not clean: 38 of the 51 still carry an unmarked bare-Hz filter corner, 41 lines** (`; dark tilt (low-pass)`, `; bright tilt (add highs)`, and the `atone …, 3500 ; the band that carries air` in five air words). That is deliberate and is BJ's ruling of 2026-07-29 — *„filter wird extern verdrahtet von parallelsession, insofern dann - anders - legitim"* — so those corners are outside this rule's reach until that wiring lands, and this paragraph records the count instead of claiming a parity that does not exist.
- **Movement by default.** Every sound moves; only an explicit "static" request is delegated to the standing-tone escape hatch. There is a **second exemption at the level of this document**, authorized by BJ on 2026-07-25: the **event-texture class**, a body that declares `; MOVEMENT: TEXTURE` in its own code — on **one standalone line, in caps**; `declares_texture` refuses the library's usual end-of-line comment style and raises on a body that merely mentions the phrase, so that the exemption cannot be granted by accident or by a negation. `lco_axis_probe.gate()` then reports its movement reading instead of failing on it, and everything else it checks still applies. The question put to BJ was whether a findable RATE in the colour track *„is the right definition for a sustained tonal body and may be the wrong one for a stochastic texture, where being alive means precisely that there is no rate to find"* — that formulation is the assistant's, not his. **His ruling** is the release: *„genau. Da haben wir nicht an Natursounds gedacht. Hiermit freigeschaltet. Ich muss ja ohnehin alle neuen instrumente reviewen."*
  - **Two further movement exemptions exist in the code and are not authorized here.** `tools/lco_param_audit.py` grants `sine` a standing exemption on its own authority ("the reference tone. A single partial with nothing to move is what this entry IS") and eight nature beds an exemption from "pitch belongs to the synth". They are entry-level exemptions in a tool, not rules of this document, and either they belong here or they belong nowhere.
  - It is a **declaration, not a second measurement**, because there is no second measurement to make: a *stationary* narrowband noise bed, in which nothing whatever moves, travels up to 1005 cents and crests at 14.55 dB, while a real sweep travels 959 — the populations overlap, so no bound on either statistic separates them. Both nulls are named constants in `tools/lco_measure.py` and the overlap is asserted in its selftest, so the threshold cannot be re-invented as a good idea. Liveness in this class rests on BJ's ear; the flag records which entries are in it. Six were: `rain`, `crackle`, `thunder`, `cymbal`, `bubbles`, `ice` — but **as of 2026-07-27 no shipped entry is in the class at all** (`--census`): five of those six are no longer in the lexicon and `cymbal` no longer declares. The rule and the mechanism stand; the population is currently empty, and the next event texture built will be the first one under it. Rationale and the measurements in `docs/parked/lco_movement_gate_three_sounds.md`.
- **The LLM is the entrance.** Every oscillator is authored by the language model. There is no deterministic route from prompt to oscillator and no "not understood" answer.
...
### Instruments 4–6 — the struck instruments (`rhodes`, `wurlitzer`, `vibraphone`) — BUILT, NOT YET HEARD

The first three keys in the library whose sound **fades by itself**, and the other half of the `wave` convention (§4). Released by BJ 2026-07-20. No parameters yet — his instruction was to get them sounding first (*„erst klingen, Parameter danach"*), so anchors are deliberately not calibrated against a timbre nobody has approved.

Each re-strikes on the trigger epoch through the same `changed2(ktrig)` / `reinit` idiom the vowel-sweep and morph emitters already use, so a new note is a new strike rather than a continuation of the last one. The opcode call sits **inside** the reinit block on purpose: these models put their whole envelope in the init pass, so re-running init is what makes the strike. Outside it, the first note of a session would ring and every note after it would be silent.

**Levels are measured, not chosen** — and one of them needed real work:

- `fmrhode` and `fmwurlie` are exemplary. `kamp` is a linear output gain (×2.00 and ×1.98 for a doubling), and peak is **flat at 0.60 across the entire register**, 55 Hz to 3520 Hz. One constant each (`0.7821`, `0.9839`) and they are done.
- `vibes` is not. `kamp` is mildly non-linear (×2.14), and peak runs **3.52 at 40 Hz down to 0.58 at 440 and back up to 0.80 at 3520** — a 15 dB tilt across the register, which is a volume control called "pitch" and precisely the defect §7 failure mode 8 exists for. Below 440 Hz it follows a clean power law (+15.4/+13.0/+10.5/+8.5/+6.5/+4.7/+2.0 dB at 40/55/80/110/160/220/320 against its own 440 Hz level); an exponent of **0.73** on (f/440) tracks it within 0.4 dB. Above 440 it is flat within ~1 dB to 3520, so the compensation is capped at 1.0 rather than fitted — a polynomial over the whole range fits 5–7 dB worse, because this is two regimes and not one curve.
- **`vibes` collapses above ~4 kHz** (−9.6 dB at 5000, −9.1 at 7000). A structural ceiling of the model, like `fmvoice`'s formant floor — not tunable, and reachable from ordinary notes at the 2' register. Recorded rather than lived with silently.

Measured through the **real generated orchestra**, not the bare opcodes, with `saw` and `fm_ep` as controls — which is what caught two harness faults that would otherwise have been read as defects in the new code:

| key | peak @110/440/1760 Hz | tail/head |
|---|---|---|
| `saw` (control) | 0.18 / 0.18 / 0.17 | 1.00 — stands |
| `fm_ep` (control) | 0.19 / 0.20 / 0.20 | 1.00 — stands |
| `rhodes` | 0.15 / 0.15 / 0.15 | **0.00** — decays |
| `wurlitzer` | 0.15 / 0.15 / 0.15 | **0.00** — decays |
| `vibraphone` | 0.13 / 0.15 / 0.17 | 0.005 — decays |

The struck keys sit ~1.6 dB under the standing ones in peak. Left there rather than trimmed: a struck sound's peak is a transient and its perceived loudness sits below it, so the honest arbiter is BJ's ear, not a peak match.

**Open:** none of the three has been heard. Whether the model reliably distinguishes "epiano" from "epiano wave" is a ROUTING question, and routing is only testable in the built Standalone.
...
Untouched by the re-measurement: `barmodel` (no frequency argument exists); `prepiano` (segfaults or hangs).

**`wgflute` — the rejection recorded here was wrong, and the opcode is still not in the library. Both halves matter, 2026-07-30.** Three numbers for the same pitch existed side by side: `csound_orch.py` said +1945 cents at 110 Hz, this section said +2999, and the probe table above shows 84 % of the energy sitting ON the comb of the asked pitch at that pitch. All three are reproducible and all three are the same thing — the pipe speaking a mode other than its first, because `kjet` was 0.30–0.32, which is one grid point from a working setting. Re-measured properly (`kjet` 0.34, `ijetrf` 0.70, `kamp` 0.64, `iminfreq` 20): the model **tracks `kfreq` from 180 Hz to 4 kHz within 25 cents**, comb contrast 28–40 dB, and it **stands** — sustain 1.002–1.060. The `sustain 0.396` in the table above is the probe's own `idetk = 0.9`, the model's decay TIME, passed on a 4 s note and then written down as a property of the opcode; at `idetk 0.01` it is 1.02. This is failure mode 9 again, in its milder form: not a call the component rejected, but a call whose arguments chose the answer.

What actually keeps it out is BJ's ear, on a full A/B on disk with the `flute` entry as the incumbent: **"extrem instabil, alle files. als SFX-Osc geht das vielleicht, aber nicht als regulärer."** The measurements do not contradict him and did not predict him, which is the useful part — and the first attempt to explain that gap after the fact was itself wrong, which is the more useful part.

That first explanation said the energy sits too high: a 220 Hz note reads 1632–2095 Hz on an ad-hoc amplitude-weighted centroid against the `flute` entry's 257–331 Hz, so the wind's 26 cents on the fundamental is 26 cents on twenty partials, heard as the top of the sound swimming. **That ordering is real but the conclusion drawn from it is not, and `centroid_audible` was not "saying 234 Hz for the same file" — it is POWER-weighted where the 1632 Hz figure was AMPLITUDE-weighted, a comparison its own docstring in `tools/lco_measure.py` forbids.** Measured against the library instead of against one entry, at 220 Hz over all 30 bodies, the pipe is inside the distribution on both: amplitude-weighted 3841 Hz against a library median of 771 and a max of 9652 (`sync`) — 87th percentile; power-weighted 238 Hz against a median of 445 and a max of 1935 — **10th** percentile, i.e. among the darkest entries there are. `flute` is simply one of the darkest incumbents, so a factor of six against it means nothing on its own.

Four further published short-window descriptors put it no further out: centroid wander (p95/p5 over 46 ms frames) 91 cents against a library range of 0–2262 (13th percentile); spectral flatness 87th/80th; energy share above the 8th partial −22.6 dB, 53rd; and the swing of that share 1.75 dB against a range of 0.01–49.3 — **7th** percentile. By every standard measure available the body is at the steady end of its own library. **So there is no cheap meter to build here, and building one would be a self-made perceptual model, which [[feedback_no_selfmade_perceptual_meters]] forbids.** What the episode actually changes is the order of work, not the instrumentation: the A/B against the incumbent decides, so it belongs before the four axes are parametrised and documented, not after.
...
10. **Reading a flat measurement as an absent effect.** A sound word was declared inert because a spectral-centroid sweep across its range came back flat. Two independent errors in one verdict: the meter was **one number wide**, and the flatness was read as a property of the word. It is not. A flat average is what **disagreement between the runs** looks like — average N runs that each moved a different parameter and the average moves nothing. Measured, not argued: Siedenburg & Saitis (2023, arXiv:2304.07830) found the standard deviation of a rated profile tracked inter-rater agreement at **r = .98** (540 human listeners) and **r = .99** (a language model), i.e. the flat profiles were exactly the ones the raters disagreed about. **A null result about a word, a parameter or an axis is not reportable without the spread beside it** — how far the individual runs differed from each other. A large spread means „the authoring is inconsistent here", which is a different problem with a different fix, and the more interesting one. This is the fourth time the meter rather than the thing produced the wrong number (7, 8, 9) and the second time it cost capability rather than accuracy: a word declared inert is a word nobody goes back to repair.
